...
void MD5Hash(char Input[128], int Size) {
	...
	for(i = 0; i < 16; i++)
	{
		sprintf_s(Input, 128, "%s%02X", Input, m_md5.digest[i]);
	}
}*/


BOOL MD5Hash(char* Input, int Size, char** ppOutHash) {
	...
	char* outHash = (char*)malloc(MD5_HASH_SIZE + 1);
	if (outHash == NULL)
		return FALSE;


	for (i = 0; i < 16; i++) {
		sprintf_s(outHash + (i * 2), MD5_HASH_SIZE + 1 - (i * 2), "%02x", m_md5.digest[i]);
	}
